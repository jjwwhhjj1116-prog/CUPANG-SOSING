...
export function prepareCollectionProduct(owner:string,job:CollectionJob,receipt:CollectionResult,id:string,now:string){
  ...
  const rows=result.options.map((row,index)=>({...emptyOptionInput(`collected-${index+1}`),originalName:row.name,supplierSku:row.sku,unitCostCny:row.unitPriceCny,minimumOrderQuantity:row.minimumOrder,included:true}));
  ...
  const options={...emptyProductOptions(id),revision:1,updatedAt:now,rows:rows.map(row=>({...row,updatedAt:now,provenance:Object.fromEntries(Object.keys(optionFieldNames).map(key=>[key,['originalName','supplierSku','unitCostCny','minimumOrderQuantity'].includes(key)?'collected':['unitsPerPack','included'].includes(key)?'manual':'unverified']))} as ProductOption))};
