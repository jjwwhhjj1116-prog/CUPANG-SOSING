import { detailImageKeys, labelDocumentRows, type ProductContent } from '@/app/product-content';
...
export function createReviewBundle(product: ProductRecord, content: ProductContent, assets: BundleAsset[], extraFiles: {name:string;data:string|Uint8Array}[] = []) {
  ...
  const labelLines=['검토용 표시사항 · 입력값 기준',...labelDocumentRows(content).flatMap(([key,name,value])=>{
    const text=(labelNames[key]||name)+': '+(value || '[미입력]');
