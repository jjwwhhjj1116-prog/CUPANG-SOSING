...
export async function POST(request: Request, context: {params: Promise<{id: string}>}) {
  ...
  try {
    ...
    if(input.action === 'preview') return json({fingerprint:revision,report,submissionReview:fields.review,filename:generated.filename,rows:rows.map(row=>mapQuotationRow(profile,row).values),headers:template.headers});
