...
import { CATEGORY_PROFILE_BODY_LIMIT, validateCategoryProfile } from '@/app/category-profiles';
...
export async function POST(request: Request) {
  if (process.env.NODE_ENV === 'production' && !(await getChatGPTUser())?.verifiedAccess) return unavailable();
  let input;
  try { input = validateCategoryProfile(await body(request)); }
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    id = value.id; expectedRevision = value.expectedRevision; input = validateCategoryProfile(value.profile);
  } catch (error) { return NextResponse.json({ error: errorText(error) }, { status: inputStatus(error), ...options }); }
  try {
    const ownerId = await owner();
