...
export function filterRegistrationProducts<T extends BoardProduct>(products:T[],query:string,from:string|null,to:string|null) {
  return products.filter(product=>{
    ...
    return `${product.title} ${product.content_summary?.seoTitle??''} ${product.source_url}`.toLowerCase().includes(query.trim().toLowerCase())
...
export function RegistrationBoard<T extends BoardProduct>({products,selected,onSelected,onOpen,onOptions,loading,error,onArchive}:{products:T[];selected:Set<string>;onSelected:(value:Set<string>)=>void;onOpen:(product:T,step?:string)=>void;onOptions?:(product:T)=>void;loading:boolean;error:string;onArchive:()=>void}) {
  ...
  return <section className="registration-board" aria-label="상품별 등록 현황">
    ...
    {rows.map(product=>{const image=registrationThumbnail(product);const title=registrationTitle(product);return <tr key={product.id}>
      ...
      {steps.map(([step,label])=><td key={step}><button type="button" className="registration-cell-button" aria-label={`${title} ${label} 열기`} onClick={()=>onOpen(product,step)}>{registrationStepLabel(product,step)}</button></td>)}
      <td><span className="registration-state">{product.registration_status}</span></td><td><button type="button" className="btn ghost" aria-label={`${title} 상세`} onClick={()=>onOpen(product)}>열기</button></td>
    </tr>;})}
