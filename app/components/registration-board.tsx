...
export function RegistrationBoard<T extends BoardProduct>({products,selected,onSelected,onOpen,loading,error,onArchive}:{products:T[];selected:Set<string>;onSelected:(value:Set<string>)=>void;onOpen:(product:T,step?:string)=>void;loading:boolean;error:string;onArchive:()=>void}) {
  ...
  return <section className="registration-board" aria-label="상품별 등록 현황">
    ...
    {rows.map(product=>{const image=thumbnail(product.image_keys);return <tr key={product.id}>
      ...
      <td className="registration-product"><button type="button" className="registration-title" onClick={()=>onOpen(product)}>{product.title}</button><button type="button" className="registration-options" onClick={()=>onOpen(product,'옵션')}>옵션 {product.options_count}개</button><a href={sourceUrl(product.source_url)} target="_blank" rel="noreferrer">{product.source_url||'원본 URL 미입력'}</a><small>공급 {product.supply_price.toLocaleString('ko-KR')}원 · 판매 {product.sale_price.toLocaleString('ko-KR')}원</small></td>
