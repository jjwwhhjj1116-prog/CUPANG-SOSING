...
export function BatchWorkPanel({products,onOpen}:{products:Item[];onOpen:(id:string)=>void}) {
  ...
  if(mode==='labels')return <div className="modal-form"><button type="button" className="btn ghost" onClick={()=>setMode('pricing')}>← 가격 계산 작업</button><BatchLabelPanel products={products} onOpen={onOpen}/></div>;
