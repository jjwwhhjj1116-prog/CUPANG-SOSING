...
export function TranslationIntegratedPreview({ productId, version, jobId, disabled, onSaved }: {
  ...
  async function run(action: 'preview' | 'apply') {
    ...
    try {
      const response = await fetch(`/api/products/${encodeURIComponent(productId)}/translation-apply`, { method: 'POST', signal: controller.signal,
        headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action, jobId, expectedVersion: version, ...(action === 'apply' ? { fingerprint: plan!.fingerprint } : {}) }) });
      ...
      if (action === 'preview') {
        ...
        setDone(true); setPlan(null); setMessage(`SEO·표시사항·옵션 ${result.applied}개 항목을 함께 저장했습니다.`); onSaved?.();
      }
  ...
  return <fieldset disabled={disabled || busy || done} aria-label="SEO 표시사항 옵션 통합 적용">
    <legend>SEO · 표시사항 · 옵션 함께 저장</legend>
    <p>완료된 같은 번역 결과를 한 번에 적용합니다. 직접 수정한 값과 공란은 보존합니다. 이미지와 Supplier Hub 전송은 포함하지 않습니다.</p>
    <button type="button" className="btn blue" onClick={() => void run('preview')}>통합 적용 미리보기 · 무료</button>
    ...
    {plan && <><table><thead><tr><th>항목</th><th>현재 값</th><th>저장할 값</th></tr></thead><tbody>{plan.preview.map((row, index) => <tr key={index}><th>{row.name}</th><td style={{ whiteSpace: 'pre-wrap' }}>{row.before || '(공란)'}</td><td style={{ whiteSpace: 'pre-wrap' }}>{row.after}</td></tr>)}</tbody></table>
      ...
      <button type="button" className="btn blue" disabled={!plan.preview.length} onClick={() => void run('apply')}>검토한 {plan.preview.length}개 항목 통합 저장</button></>}
  </fieldset>;
}
