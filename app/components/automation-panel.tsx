...
export function AutomationPanel({productId,version}:{productId:string;version:string}) {
  ...
  return <div className="panel-stack">
    ...
    {view.workflow&&<><p>작업 버전 {view.workflow.revision} · {statuses[view.workflow.status]}</p><ol className="automation-list">{view.workflow.stages.map(stage=><li key={stage.id}><div><strong>{stage.label}</strong><span className={'status '+(stage.status==='complete'?'success':'warning')}>{statuses[stage.status]}</span></div>{stage.reason&&<p>{stage.reason.message}</p>}{stage.artifacts.map(artifact=>artifact.kind==='priceCalculation'?<PriceArtifact key={artifact.id} artifact={artifact}/>:<details key={artifact.id}><summary>{artifact.label}</summary><pre>{JSON.stringify(artifact.data??{storageKey:artifact.storageKey},null,2)}</pre></details>)}<small>시도 {stage.attempts}회 · 근거 {stage.evidence.length}개</small></li>)}</ol></>}
