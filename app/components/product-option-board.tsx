...
import {quotationMainImageKeys,optionQuotationName,type ProductOptionsResponse} from '@/app/product-options';

type Props={productId:string;sourceUrl:string;imageKeys:string;onQuotation:(optionId:string)=>void;onEdit:(optionId?:string)=>void;onImage?:(optionId:string)=>void};
type OptionBoardData=ProductOptionsResponse & {commonImageKeys:string[]};
export async function readOptionBoard(productId:string,signal:AbortSignal,fetcher:typeof fetch=fetch):Promise<OptionBoardData>{
  ...
  if(content?.productId!==productId||content.schemaVersion!==1||!Array.isArray(content.assets?.main?.value)||content.assets.main.value.some((key:unknown)=>typeof key!=='string'))throw Error('상품의 공통 이미지 응답을 확인하지 못했습니다.');
  return {...body,commonImageKeys:content.assets.main.value};
}
export function ProductOptionBoard({productId,sourceUrl,imageKeys,onQuotation,onEdit,onImage}:Props){
  ...
  return <section className="option-board" aria-busy={!data&&!error}>
    ...
    {error?<p role="alert">{error}</p>:!data?<p role="status">저장된 옵션·가격을 불러오는 중…</p>:<div className="table-wrap"><table className="option-work-table"><thead><tr><th>옵션번호 / SKU</th><th>옵션명</th><th>옵션 이미지</th><th>구성 수량</th><th>재고 / 가격</th><th>견적 포함</th><th>가격설정</th><th>견적서</th></tr></thead><tbody>{rows.map(row=>{
