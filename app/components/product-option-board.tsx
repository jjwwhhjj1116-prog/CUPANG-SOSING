...
type Props={productId:string;sourceUrl:string;imageKeys:string;onQuotation:(optionId:string)=>void;onEdit:(optionId?:string)=>void;onImage?:(optionId:string)=>void;onContent?:(step:CommonContentStep)=>void};
type CommonContentStep='추가 이미지'|'상세 이미지'|'표시사항';
const commonStages=[['additional','추가 이미지'],['detail','상세 이미지'],['label','표시사항']] as const;
type OptionBoardData=ProductOptionsResponse & {commonImageKeys:string[];commonAssets?:Record<'additional'|'detail'|'label',string[]>};
export async function readOptionBoard(productId:string,signal:AbortSignal,fetcher:typeof fetch=fetch):Promise<OptionBoardData>{
  ...
  const readKeys=(role:'additional'|'detailTop'|'detail'|'detailBottom'|'label')=>{
    const keys=content.assets[role]?.value??[];
    if(!Array.isArray(keys)||keys.some(key=>typeof key!=='string'))throw Error('상품의 공통 이미지 응답을 확인하지 못했습니다.');
    return keys;
  };
  return {...body,commonImageKeys:content.assets.main.value,commonAssets:{additional:readKeys('additional'),detail:[...readKeys('detailTop'),...readKeys('detail'),...readKeys('detailBottom')],label:readKeys('label')}};
}
export function ProductOptionBoard({productId,sourceUrl,imageKeys,onQuotation,onEdit,onImage,onContent}:Props){
  ...
  return <section className="option-board" aria-busy={!data&&!error}>
    ...
    {error?<p role="alert">{error}</p>:!data?<p role="status">저장된 옵션·가격을 불러오는 중…</p>:<div className="table-wrap"><table className="option-work-table"><thead><tr><th>옵션번호 / SKU</th><th>옵션명</th><th>옵션 이미지</th>{commonStages.map(([role,step])=><th key={role}>공통 {step}</th>)}<th>구성 수량</th><th>재고 / 가격</th><th>견적 포함</th><th>가격설정</th><th>견적서</th></tr></thead><tbody>{rows.map(row=>{
