...
export function PriceEditor({ sourcePrice, initial, onSave }: { sourcePrice: number; initial: PricePolicy; onSave: (policy: PricePolicy) => Promise<void> }) {
  ...
  return <form className="panel-stack" onSubmit={async event=>{event.preventDefault();if (busy || error) return;setBusy(true);setMessage('');try {await onSave(policy);setSavedPolicy(policy);setMessage('가격을 저장했습니다. 견적서에 반영됩니다.');} catch(e) {setMessage(e instanceof Error ? e.message : '저장 실패');}finally{setBusy(false);}}}>
