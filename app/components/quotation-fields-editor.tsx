...
function QuotationFieldsForm({ navigationTarget, productId, profileId, refreshToken, onSaved, onDirtyChange }: Props) {
  ...
  function edit(fieldKey: string, value: string | null) {
    if (!view) return;
    setChanges(previous => updateQuotationEditorDraft(view.overrides, previous, { fieldKey, optionId: selectedOption, value }));
    setConflicts(previous => previous.filter(conflict => conflict.key !== quotationEditorKey(selectedOption, fieldKey)));
  ...
  return <section className="quotation-fields" aria-busy={loading || busy}>
    ...
    {view && <>
      ...
      <QuotationTranslatedAttributes key={`translation:${view.inputFingerprint}:${view.revision}:${selectedOption??'common'}`} productId={productId} view={view} optionId={selectedOption} disabled={dirty || loading || busy || conflicts.length > 0} onApply={items=>items.forEach(item=>edit(item.fieldKey,item.value))} />
