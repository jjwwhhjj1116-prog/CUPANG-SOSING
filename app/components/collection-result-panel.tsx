...
function CollectionResultContent({jobId,productId,onSaved}:Props){
 ...
 if(!response.ok)throw new Error(body.error||'결과 조회 실패');const received=body.receipt?.result??null;setResult(received);setSelectedImages(received&&received.images.length<=50?received.images.map((_,index)=>index):[]);setMessage(productId?'상품에 반영한 원문입니다. 번역·이미지 다운로드·등록은 별도 단계입니다.':body.message);
 }catch(cause){setMessage(cause instanceof Error?cause.message:'결과 조회 실패');}finally{setBusy(false);}}
 return <details className="collection-receipt"><summary>수집 원문 확인</summary><button type="button" className="btn ghost" disabled={busy} onClick={()=>void load()}>{busy?'확인 중…':'수신 결과 조회'}</button>{message&&<p role="status">{message}</p>}{result&&<><h4>{result.title}</h4><button type="button" className="btn primary" disabled={busy||!!productId} onClick={()=>void promote()}>{productId?"상품 반영됨":"원문을 상품·옵션으로 반영"}</button><p>{result.provider} · {result.collectedAt}</p><a href={result.sourceUrl} target="_blank" rel="noreferrer">{result.sourceUrl}</a><div className="table-wrap"><table><thead><tr><th>SKU</th><th>원문 옵션</th><th>원문 색상</th><th>원문 사이즈</th><th>원가 CNY</th><th>최소 주문</th><th>재고</th><th>옵션 원본 이미지</th></tr></thead><tbody>{result.options.map(row=><tr key={row.sku}><td>{row.sku}</td><td>{row.name}</td><td>{row.color||'미확인'}</td><td>{row.size||'미확인'}</td><td>{row.unitPriceCny}</td><td>{row.minimumOrder}</td><td>{row.stock??'미확인'}</td><td>{row.imageIndex===undefined?'연결 정보 없음':`${row.imageIndex+1}번 · ${selectedImages.includes(row.imageIndex)?'저장 선택됨':'저장 미선택'}`}</td></tr>)}</tbody></table></div><p>이미지 주소 {result.images.length}개 · 원본 저장과 번역은 별도입니다.</p><fieldset disabled={busy} style={{border:0,padding:0}}><legend>저장할 원본 이미지 선택 · {selectedImages.length}/{result.images.length}개</legend><p>최대 50개를 선택하세요. 기존 상품 이미지와 합친 저장 한도도 50개입니다. 실패한 이미지는 선택을 해제하고 재시도할 수 있습니다.</p><button type="button" className="btn ghost" disabled={result.images.length>50} onClick={()=>setSelectedImages(result.images.map((_,index)=>index))}>전체 선택</button><button type="button" className="btn ghost" onClick={()=>setSelectedImages([])}>선택 해제</button><div style={{maxHeight:240,overflowY:'auto'}}>{result.images.map((image,index)=><div key={image.url}><label><input type="checkbox" checked={selectedImages.includes(index)} disabled={!selectedImages.includes(index)&&selectedImages.length>=50} onChange={event=>setSelectedImages(previous=>event.target.checked?[...previous,index]:previous.filter(value=>value!==index))}/>{index+1}번 · {image.role==='main'?'대표':image.role==='additional'?'추가':'상세'}</label> <a href={image.url} target="_blank" rel="noreferrer" style={{overflowWrap:'anywhere'}}>{image.url}</a></div>)}</div></fieldset><button type="button" className="btn ghost" disabled={busy||(result.images.length>0&&selectedImages.length===0)} onClick={()=>void importAll()}>상품·선택 이미지 {selectedImages.length}개 반영 · 재시도</button>{importing&&<button type="button" className="btn ghost" disabled={stopping} onClick={()=>{stop.current=true;setStopping(true);}}>{stopping?"현재 저장을 마친 뒤 중단합니다":"연속 작업 중단"}</button>}<pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{result.description}</pre></>}</details>;
}
