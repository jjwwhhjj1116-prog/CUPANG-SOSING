...
export function CollectionResultPanel({jobId,productId,onSaved}:{jobId:string;productId?:string|null;onSaved:()=>void}){
 ...
 async function importImages(){if(!result)return;setBusy(true);let completed=0;try{for(let index=0;index<result.images.length;index++){
  setMessage(`원본 이미지 ${index+1}/${result.images.length} 저장 중…`);
  const response=await fetch(`/api/collection-jobs/${encodeURIComponent(jobId)}/images`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({index})});
  const body=await response.json() as {error?:string};if(!response.ok)throw new Error(body.error||'이미지 저장 실패');completed++;
 }setMessage(`원본 이미지 ${completed}개 저장 확인. 기존 수동 선택은 유지됩니다. 번역은 아직 실행하지 않았습니다.`);
 }catch(cause){setMessage(`${completed}개 저장 확인 후 중단: ${cause instanceof Error?cause.message:'저장 실패'} 다시 실행하면 완료된 항목은 재사용합니다.`);}finally{setBusy(false);onSaved();}}
