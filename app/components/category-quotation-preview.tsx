...
export function CategoryQuotationPreview({ schema }: { schema: QuotationSchema }) {
  ...
  const visibleFields=schema.fields.filter(field=>{
    ...
    return (filter==='all'||filter==='required'&&field.required||filter==='known'&&known||filter==='unknown'&&!known||filter==='linked'&&quotationInputLink(field)!==null)
  ...
  return <details className="category-quotation-preview">
    ...
    <p>{schema.evidence}</p>
    <p>작성 순서: 옵션별 직접 수정 → 공통 직접 수정 → 아래 연결된 저장값 → 확인된 쿠플러스 기본값. 직접 비운 값도 유지됩니다. 화살표 뒤 값은 앞의 값이 미입력일 때만 사용합니다.</p>
    <div className="workspace-actions">
      ...
      <label>항목 보기<select value={filter} onChange={event=>setFilter(event.target.value)}><option value="all">전체 항목</option><option value="required">필수 항목</option><option value="linked">단계별 자동 연결 항목</option><option value="known">확인된 기본값</option><option value="unknown">미확인 기본값</option></select></label>
    ...
    {Object.entries(quotationSections).map(([section, title]) => {
      ...
      return <details key={`${section}:${filter}:${normalized}`} open={filter!=='all'||!!normalized||undefined}><summary>{title} · {fields.length}개</summary>
        <div className="category-quotation-table"><table><caption>{title} 견적 항목</caption><thead><tr><th scope="col">항목</th><th scope="col">입력 기준</th><th scope="col">자동작성에 연결되는 입력</th><th scope="col">쿠플러스에서 확인한 기본값</th></tr></thead>
