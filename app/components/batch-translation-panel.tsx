...
export function BatchTranslationPanel({ products, onOpen }: { products: Item[]; onOpen: (id: string) => void }) {
  ...
  return <section className="panel-stack" aria-label="선택 상품 번역 통합 검토" aria-busy={busy||batchBusy}>
    ...
    {products.map(product=>{
      ...
      return <section key={product.id} className="translation-review"><h4>{product.title}</h4>
        ...
        {target?.jobId&&<TranslationIntegratedPreview key={`${key}:${target.jobId}`} productId={target.productId} version={target.version} jobId={target.jobId} disabled={busy||batchBusy}/>}
        <button type="button" className="btn ghost" disabled={busy||batchBusy} onClick={()=>onOpen(product.id)}>상품별 작업 열기</button>
      </section>;
    })}
  </section>;
}
