...
현재 저장 위치는 collection_results이며 요청 당시 category/settings 문맥은 기존 collection_context에 보존됩니다. 원문을 상품/옵션/콘텐츠로 승격하는 트랜잭션, 이미지 다운로드/R2, 실제 공급원 인증/실행은 다음 단계입니다. 수신만으로 상품 생성, 수집완료 표시, 결제, 등록전송을 수행하지 않습니다. 외부 사이트에 접근하지 않는 합성 API/SQLite 테스트를 실수집 성공으로 해석하면 안 됩니다.
