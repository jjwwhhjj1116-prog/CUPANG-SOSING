...
test('batch view isolates product failures and links exact reviewed result without executing a provider',async()=>{
 ...
 const {BatchTranslationPanel}=load('app/components/batch-translation-panel.tsx',{react:hooks,'@/app/components/translation-integrated-preview':{TranslationIntegratedPreview:Preview},'@/app/batch-translation':{readBatchTranslationTarget:async id=>{calls.push(id);if(id==='bad')throw Error('개별 조회 실패');return{productId:id,version,jobId:id==='empty'?null:job.id};}},fetch:()=>{throw Error('no provider');}});
